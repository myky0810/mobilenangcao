...
class GooglePhoneRegistrationScreen extends StatefulWidget {
  const GooglePhoneRegistrationScreen({super.key, required this.firebaseUser});
...
class _GooglePhoneRegistrationScreenState
  ...
  Future<void> _handlePhoneSubmit() async {
    ...
    if (phone.length != 9) {
      ...
      AppSnackBar.show(
        context,
        'Nhập 9 chữ số sau +84 (ví dụ: 374854273)',
    ...
    try {
      ...
      AppSnackBar.show(context, errorMsg, backgroundColor: Colors.red);
    } catch (e) {
      print('❌ Error: $e');
      if (!mounted) return;
      setState(() => _isLoading = false);
